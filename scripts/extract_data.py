...
def main(symbols: List[str], requests: List[str], queries: dict = {}, save_to: str = None):
    """
    ...
    if save_to.lower() != "none":
        save_raw_data(data, symbols=symbols, requests=requests, save_to=save_to)
...
if __name__ == "__main__":
    ...
    if args.config:
        ...
        symbols, requests, queries, save_to = parse_inputs(**config)

    # If arguments were passed through CLI
    if args.manual:
        symbols, requests, queries, save_to = parse_inputs(args.symbols, args.requests, args.queries, args.save_to)
    ...
    python fetch.py --manual --symbols AAPL MSFT GOOGL --requests stock_statement cashflow --queries "from=2022-05-01" "to=2023-05-01" "period=quarter" "limit=4" --save_to timestamp
    '''
    
    data = main(symbols=symbols, requests=requests, queries=queries, save_to=save_to)
